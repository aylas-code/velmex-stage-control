classdef serialport_VMX < handle & matlab.mixin.CustomDisplay
    properties
        vmx = [];
        port = 11;
        motorResponse = [];
    ...
    methods
        ...
        function obj = serialport_VMX(portNum, motorSpeed, xNum, yNum, thetaNum, zNum)
            if nargin > 0
        ...
        function setSpeed(obj)
            for ii = 1:length(obj.motorsPresent)
                ...
                write(obj.vmx,speedCommand,'char');  % Changed from fprintf to write
                pause(.1); % pause for 100ms
                obj.motorResponse = read(obj.vmx,1,'char');  % Changed from fscanf to read
            end
        end
        ...
        function toggleUnits(obj,axisToToggle)
            ...
            if strcmpi(axisToToggle,'theta')
                if strcmpi(obj.(axisToToggle).units,'degrees')
                    % convert to radians
                    newUnits = 'radians';
                    newUnitsPerStep = 0.0125/400*pi/180;
                else
                    % convert to degrees
                    newUnits = 'degrees';
                    newUnitsPerStep = 0.0125/400;
                end
            else
        ...
        function delete(obj)
            if ~isempty(obj.vmx)
                write(obj.vmx,'Q','char'); % release the motor  % Changed from fprintf to write
                delete(obj.vmx); % Removed fclose, only need delete for serialport
            end
        end
    ...
    methods (Access = private)
        ...
        function connectMotor(obj)
            if isempty(obj.vmx)
                obj.vmx = serialport(sprintf('COM%d',obj.port), obj.baud, 'DataBits', 8, 'Parity', 'none', 'StopBits', 1);  % Changed from serial to serialport
                configureTerminator(obj.vmx,"CR/LF");
                obj.vmx.Timeout = 30;  % Changed from set(obj.vmx,'Timeout',30)
                
                % send a verify command
                write(obj.vmx,'V','char');  % Changed from fprintf to write
        ...
        function moveMotor(obj,motorToMove,stepsToMove)
            ...
            write(obj.vmx, sprintf('F C %s, R', moveCommand), 'char');  % Changed from fprintf to write
            pause(.1); % pause for 100ms
